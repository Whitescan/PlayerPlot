...
public class ProtectionUtil {
	...
	public static boolean isProtectedInteraction(Material handMaterial, Material blockMaterial) {
		return INTERACTABLE_MATERIALS.contains(blockMaterial) || isStripLogAttempt(handMaterial, blockMaterial)
				|| isPlaceableItem(handMaterial) || isUsableItem(handMaterial);
	}
	...
	private static ImmutableSet<EntityType> INTERACTABLE_AT_ENTITIES = ImmutableSet.<EntityType>builder()
			.add(EntityType.ARMOR_STAND).build();
	...
	private static final ImmutableSet<EntityType> INTERACTABLE_ENTITIES = ImmutableSet.<EntityType>builder()
			.add(EntityType.ITEM_FRAME).add(EntityType.VILLAGER).add(EntityType.LEASH_HITCH)
			.add(EntityType.MINECART_HOPPER).add(EntityType.MINECART_COMMAND).add(EntityType.MINECART_CHEST)
			.add(EntityType.MINECART_FURNACE).add(EntityType.MINECART_MOB_SPAWNER).build();
	...
	private static final ImmutableSet<Material> PLACEABLE_ITEMS = ImmutableSet.<Material>builder()
			.add(Material.ITEM_FRAME).add(Material.PAINTING).build();

	private static boolean isUsableItem(Material handItem) {
		return USABLE_ITEMS.contains(handItem);
	}

	private static ImmutableSet<Material> USABLE_ITEMS = ImmutableSet.<Material>builder().add(Material.WATER_BUCKET)
			.add(Material.LAVA_BUCKET).add(Material.FLINT_AND_STEEL).add(XMaterial.BONE_MEAL.parseMaterial()).build();
	...
	private static Set<EntityType> buildMonstersMobs() {
		...
		for (XEntityType entityType : xEntityTypes) {
			if (entityType.isSupported())
				monsters.add(entityType.parseEntityType());
		}
	...
	public static boolean isFighting(Entity entity) {
		return entity instanceof Creature && AGGROABLE_MOBS.contains(entity.getType())
	...
	private static Set<Material> buildSmallMaterials() {
		...
		for (XMaterial xMaterial : xMaterials) {
			if (xMaterial.isSupported())
				smallMaterials.add(xMaterial.parseMaterial());
		}
