...
public class FilePlotDatabase implements IPlotDatabase {
	...
	public void store(UUID plotID, Plot plot) {
		...
		if (plot != null) {
			...
			String baseString = plot.getName() + "=" + plot.getOwnerName() + "=" + plot.getOwnerId() + "="
					+ plot.getWorld() + "=" + minCorner.getX() + "_" + minCorner.getZ() + "<" + maxCorner.getX() + "_"
					+ maxCorner.getZ() + "=" + plot.getComponents();
	...
	public List<Plot> fetch() {
		...
		for (String plotIDString : config.getRoot().getKeys(false)) {
			try {
				...
				LocationParts spawn = config.contains(key + ".spawn")
						? LocationParts.parseParts(config.getString(key + ".spawn"))
						: null;
