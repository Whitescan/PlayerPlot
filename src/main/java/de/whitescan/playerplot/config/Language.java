...
public enum Language {
	...
	PLUGIN_DESCRIPTION("Plugin - description",
			"Player Plot is a self-serve protection plugin. It allows users to unlock and claim protected regions."),
	PLUGIN_READ_MORE("Plugin - read more", "Read more on the [link]."),
	PLUGIN_WIKI("Plugin - wiki", "Player Plot wiki"), PLUGIN_OPTIONS("Plugin - options", "Options"),
	PLUGIN_AUTHOR("Plugin - author", "Author"), PLUGIN_VERSION("Plugin - version", "Version"),
	PLUGIN_RELOAD("Plugin - reload", "Reload complete."),

	LABEL_COMMANDS("Label - commands", "Commands"), LABEL_PLOTS("Label - plots", "Plots"),
	LABEL_FRIEND_PLOTS("Label - friend plots", "Friend Plots"), LABEL_PAGE("Label - page", "Page"),
	LABEL_AREA("Label - area", "Area"), LABEL_CENTER("Label - center", "Center"),
	LABEL_MIN_CORNER("Label - min corner", "Min Corner"), LABEL_MAX_CORNER("Label - max corner", "Max Corner"),
	LABEL_WORLD("Label - world", "World"), LABEL_COMPONENTS("Label - components", "Components"),
	LABEL_FRIENDS("Label - friends", "Friends"), LABEL_PLOT_DEED("Label - plot deed", "Plot Deed"),
	...
	LABEL_PLOT_DEEDS("Label - plot deeds", "Plot Deeds"), LABEL_ALL_PLOTS("Label - all plots", "All Plots"),
	LABEL_CONFIRM("Label - confirm", "Confirm"), LABEL_CANCEL("Label - cancel", "Cancel"),
	...
	HELP_PLOT_SCAN("Help - plot scan", "display plot boundary"), HELP_PLOT_CLAIM("Help - plot claim", "claim a plot"),
	HELP_PLOT_RENAME("Help - plot rename", "rename a plot"), HELP_PLOT_FREE("Help - plot free", "delete a plot"),
	HELP_PLOT_INFO("Help - plot info", "get plot details"), HELP_PLOT_LIST("Help - plot list", "list your plots"),
	...
	HELP_TOPLOT("Help - toplot", "teleport to plot"), HELP_PLOT_DEED("Help - plotdeed", "give plot deeds to player"),
	HELP_WRITE_DEED("Help - writedeed", "write plot deeds"), HELP_ALL_PLOTS("Help - allplots", "view all plots"),
	HELP_DEL_PLOT("help - delplot", "delete any plot"),

	CONSOLE_DETECT("Console - plugin detected", "%plugin% detected"),

	ARG_AMOUNT("Arg - amount", "amount"), ARG_PLAYER("Arg - player", "player"), ARG_NAME("Arg - name", "name"),
	ARG_PLOT("Arg - plot", "plot"), ARG_PLOT_DEED("Arg - plot deed", "plot deed"),
	...
	MISC_HERE("Misc - here", "here"), MISC_FORMAT("Misc - format", "Format is %format%"),
	...
	MISC_OPTIONAL("Misc - optional", "optional"), MISC_VARIABLE("Misc - variable", "variable"),
	...
	STATUS_SPECIAL_CHARACTERS("Status - name special characters",
			"Plot names must consist of only a-z, A-Z, 0-9, _, and -"),
	...
	public String coloredFromPlayerAndPlot(String playerName, String plotName, ChatColor base, ChatColor highlight) {
		return base + get().replaceAll("%player%", highlight + playerName + base).replaceAll("%plot%",
				highlight + plotName + base);
	}
	...
	public TextComponent getWithPlayerConfirmDeny(ChatColor baseColor, ChatColor playerColor, String playerName,
			String confirmCommand, String cancelCommand) {
		String string = get();
		...
		if (Version.getValue() >= 116.2) {
			confirmComponent.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
					new Text(ChatColor.GREEN + Language.INFO_CONFIRM_HOVER.toString())));
		} else {
			confirmComponent.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
					new ComponentBuilder(Language.INFO_CONFIRM_HOVER.toString()).color(ChatColor.GREEN.asBungee())
							.create()));
		}
		...
		if (Version.getValue() >= 116.2) {
			cancelComponent.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
					new Text(ChatColor.RED + Language.INFO_CANCEL_HOVER.toString())));
		} else {
			cancelComponent.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
					new ComponentBuilder(Language.INFO_CANCEL_HOVER.toString()).color(ChatColor.RED.asBungee())
							.create()));
		}
		...
		for (int i = 0; i < confirmSubs.length; i++) {
			...
			if (i < confirmSubs.length - 1 || string.endsWith("[confirm]"))
				base.addExtra(confirmComponent);
